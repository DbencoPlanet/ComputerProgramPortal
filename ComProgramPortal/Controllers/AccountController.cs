...
namespace ComProgramPortal.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public AccountController(ApplicationUserManager userManager, ApplicationSignInManager signInManager, UserManagerService userService)
        {
                ...
                case SignInStatus.Success:
                    if (user.IsLocked != true)
                    {
                        ...
                        else
                        {
                            ...
                            else if (await SignInManager.UserManager.IsInRoleAsync(user.Id, "Teacher"))
                            {
        ...
        //
        // GET: /Account/UserLockout

        public async Task<ActionResult> UserLockout(string userid)
        {

            var staff = await db.TeacherProfiles.Include(x => x.user).FirstOrDefaultAsync(x => x.UserId == userid);
            if (staff != null)
            {
                ViewBag.user = staff;
            ...
            {
                var student = await db.StudentInfos.Include(x => x.user).FirstOrDefaultAsync(x => x.UserId == userid);
